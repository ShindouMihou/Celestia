<script lang="ts">
    ...
    onMount(async () => {
        ...
        load(() => scroller.observe(document.querySelector("#__tunnelVision")));
    });
     ...
     * into the page.
     */
    async function load(onFulfilled = () => {}) {
        ...
        loading = true;
        request(`/api/glassbox/${glassbox}`, (response) => {
        ...
        if (next) {
            ...
            request(next, (response) => {
                ...
                window.history.pushState(
                    {},
                    "",
                    `/dashboard/${glassbox}${url.stringify(url.searchParams)}`
                );
            ...
            request(
                ...
                (response) => {
                    ...
                    url.searchParams.set(
                        "query",
                        btoa(JSON.stringify(decoder(query)))
                    );
                    window.history.pushState(
                        {},
                        "",
                        `/dashboard/${glassbox}${url.stringify(
                            url.searchParams
                        )}`
                    );
    ...
    <div class="rounded w-full border">
        <div class="flex flex-col p-3 gap-2">
            <div class="flex flex-row gap-1 items-center">
                ...
                <a
                    href="https://github.com/ShindouMihou/Celestia/blob/master/documentations/filters.md"
                    rel="external"
                    target="_blank"
                >
                    <Icon
                        src={QuestionMarkCircle}
                        class="h-4 w-4 hover:text-blue-500 duration-300"
                    />
                </a>
            </div>
            <textarea
                class="p-4 text-md font-light bg-gray-100 rounded outline-none"
                placeholder="FIELD=VALUE"
                id="_filter"
            />
            <button class="cursor-pointer" on:click={filter} transition:fade>
                <div
                    class="bg-slate-100 border text-black rounded p-2 flex flex-col hover:opacity-80 gap-4 lg:w-32"
                >
    ...
    <div class="flex flex-col md:flex-row gap-4 w-full">
        ...
        <div class="border rounded md:w-96 flex-grow">
            <div class="flex flex-col gap-4 p-4">
                ...
                <div
                    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2 gap-2"
                    id="documents"
                >
                    {#if documents == null}
                        {#each { length: 5 } as _, i}
                ...
                {#if loading}
                    <div
                        class="w-full align-middle justify-center flex flex-row gap-4 m-auto py-6 animate-bounce"
                    >
